...
public class LoginDAO {
	...
	public UserDistrictCycleYearList getAllowedDistrictsCycleYear_NEW(UserCredentialsFromAndroidBean model) {
		...
		UserDistrictCycleYearList respose = jdbcTemplate.query(sql_check, params_check, rs -> {
			...
			UserDistrictCycleYearList temp_inst = new UserDistrictCycleYearList(new ArrayList<>(), new ArrayList<>(),
					new ArrayList<>());
		...
		// In case if user is admin then his district allotted is 0. Also he will have
		// only 1 district i.e. 0.
...
//					respose.setCycle_list(clist);
			...
			String SQL_districts = "SELECT d.`district_id`,d.`country_id`,d.`district_name`,d.`state_id`,s.`region_id` FROM `district` d inner join `country_states` s on d.state_id=s.cs_id  where `district_id` IN ("
					+ dstr + ");";
